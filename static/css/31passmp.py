...
def compute_portvals(orders_file = "./orders/orders.csv", start_val = 1000000, commission=9.95, impact=0.005):
    ...
    for index, row in orders_df.iterrows():
        ...
        if row['Order'] == "BUY":
            ...
            trades_df.loc[row['Date'], "CASH"] = trades_df.loc[row['Date'], "CASH"] + (
                        row['Shares'] * prices_df.loc[row['Date'], traded_share] * (1 + impact) * (-1)) - (commission)
        elif row['Order'] == "SELL":
            trades_df.loc[row['Date'], row['Symbol']] += (row['Shares'] * (-1))
            trades_df.loc[row['Date'], "CASH"] = trades_df.loc[row['Date'], "CASH"] + (
                        row['Shares'] * prices_df.loc[row['Date'], traded_share] * (1 - impact)) - (commission)
